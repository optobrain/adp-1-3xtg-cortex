...
for id=1:nd
    ...
    tb1.frc = l.frac;
    if numel(l.bn) == 1
        error("bn has only one value in %s.", tbMeta1.did);
    end
    tb1.bran = mean(l.bn .* (1:numel(l.bn)));
    tb1.cycMean = l.cyc;  % missing in old mice. Asked Jang
    tb1.cycCov = l.cyc_cov;
    tb1.denLen = mean(l.len_den);  % now it is an array. Asked Sabina
    tb1.denNum = mean(l.num_den);
...
met = ["len", "dia", "tor", "bran", "btw", "cls", "cyc", "denNum", "denLen", "frc"];
% metName = ["Length", "Diameter", "Tortuosity", "Branching order", "Betweenness", "Closeness", "Shortest cycle", "Number density", "Lenght density", "Fractal dimension"];
% met = ["len", "dia", "tor", "bran", "btw", "cls", "cyc", "frc"];
metName = ["Length", "Diameter", "Tortuosity", "Branching order", ...
    "Betweenness", "Closeness", "Shortest cycle", ...
    "Number density", "Length density", "Fractal dimension"];
mtype = ["Mean", "Cov"];
mtypeName = ["Mean", "COV"];
nm = numel(met);
...
%         else
            ...
            fprintf("%s effect size: %.1f to %.1f %% \n", met1, effectSize.ci(1), effectSize.ci(2));
%         end
...
%         ax.XLabel.String = xl;
%         ax.Title.String = sprintf("%s %s", metName(im), mtypeName(it));
%         ax.YLabel.String = "Relative change (%)";
        ax.Title.String = metName(im);
        ax.YLabel.String = sprintf("%s change (%%)", mtypeName(it));
