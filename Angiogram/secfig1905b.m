...
vdfitmean = est(1) + est(2) ./ (1+exp(-(ti-t0)/(tau/5.89)));
vdfitse = sqrt( se(1)^2 + ( se(2) ./ (1+exp(-(ti-t0)/(tau/5.89))) ).^2 + 2*covr(1,2)./(1+exp(-(ti-t0)/(tau/5.89))) );
...
%     line(ti(idx+1)*[1 1],get(gca,'ylim'),'color','r','linestyle',':');
    ...
    disp(['No significant difference from week ' num2str(t(1))]);
end
