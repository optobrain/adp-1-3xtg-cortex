function [S1, exit_flag] = clickT(S1,out,i,col,xx,yy, numtile)
    ...
    handles.NumberTile = uicontrol('Style','Text','Position',[350 450 100 40],'String',['Tile number: ' num2str(numtile(1)) '/' num2str(numtile(2))]);
...
function pb_kpf(varargin)
    ...
    if strcmp(varargin{1,2}.Key, 'x') 
        ...
        %dat.skel = seg2skel(dat.S1, dat.skel);
        %dat.mp = find_ep(dat.skel, dat.S1);
        for j2 = 1:length(ind)
            s = dat.S1{ind(j2)};
